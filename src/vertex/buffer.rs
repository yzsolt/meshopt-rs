...
fn encode_bytes_group_measure(buffer: &[u8], bits: usize) -> usize {
	...
	match bits {
		1 => if encode_bytes_group_zero(buffer) { 0 } else { usize::MAX }
		8 => BYTE_GROUP_SIZE,
		_ => {
			...
			result += &buffer[0..BYTE_GROUP_SIZE].iter().map(|b| (*b >= sentinel) as usize).sum();
...
fn encode_vertex_block(mut data: &mut [u8], vertex_data: &[u8], vertex_count: usize, vertex_size: usize, last_vertex: &mut [u8; 256]) -> Option<usize> {
	...
	for k in 0..vertex_size	{
		...
		let written = encode_bytes(data, &buffer[0..(vertex_count + BYTE_GROUP_SIZE - 1) & !(BYTE_GROUP_SIZE - 1)])?;
	...
	W: Write,
{
	...
	let mut next = |bits, data: &mut Cursor<&[u8]>, byte: &mut u8, data_var_pos: &mut u64| {
		...
		*byte <<= bits;
		...
		write_byte(buffer, if enc == (1 << bits) as u8 - 1 {
			encv
		} else { 
			enc
		});
	...
	match bitslog2 {
		0 => { buffer.write(&[0; BYTE_GROUP_SIZE]).unwrap(); }
		1 => {
...
fn decode_vertex_block(data: &mut Cursor<&[u8]>, vertex_data: &mut [u8], vertex_count: usize, vertex_size: usize, last_vertex: &mut [u8; 256]) -> Result<(), DecodeError> {
	assert!(vertex_count > 0 && vertex_count <= VERTEX_BLOCK_MAX_SIZE);
...
pub fn encode_vertex_buffer<Vertex>(buffer: &mut [u8], vertices: &[Vertex], version: VertexEncodingVersion) -> Option<usize> {
	...
	while vertex_offset < vertices.len() {
		...
		let written = encode_vertex_block(&mut data, &vertex_data[vertex_offset * vertex_size..], block_size, vertex_size, &mut last_vertex)?;
		data = &mut data[written..];
...
pub fn decode_vertex_buffer<Vertex>(destination: &mut [Vertex], buffer: &[u8]) -> Result<(), DecodeError> {
	...
	while vertex_offset < vertex_count {
		...
		decode_vertex_block(&mut data, &mut vertex_data[vertex_offset * vertex_size..], block_size, vertex_size, &mut last_vertex)?;
...
mod test {
	...
	const VERTEX_BUFFER: [PackedVertexOct; 4] = [
		PackedVertexOct { p: [0, 0, 0],     n: [0, 0], t: [0, 0] },
		PackedVertexOct { p: [300, 0, 0],   n: [0, 0], t: [500, 0] },
		PackedVertexOct { p: [0, 300, 0],   n: [0, 0], t: [0, 500] },
		PackedVertexOct { p: [300, 300, 0], n: [0, 0], t: [500, 500] },
	];
	...
	fn encode_test_vertex() -> Vec<u8> {
		let mut buffer = vec![0; encode_vertex_buffer_bound(VERTEX_BUFFER.len(), std::mem::size_of::<PackedVertexOct>())];
