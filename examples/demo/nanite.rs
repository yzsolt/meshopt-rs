...
fn partition(clusters: &[Cluster], pending: &[i32]) -> Vec<Vec<i32>> {
    ...
    for p in pending {
        if result.is_empty() || last_indices + clusters[*p as usize].indices.len() > CLUSTER_SIZE * 4 * 3 {
            result.push(Vec::new());
            last_indices = 0;
        }
...
fn simplify2(vertices: &[impl Vertex], indices: &[u32], target_count: usize, error: Option<&mut f32>) -> Vec<u32> {
    ...
    let size = simplify(&mut lod, indices, vertices, target_count, f32::MAX, options, error);
    lod.truncate(size);
...
pub fn nanite(vertices: &[impl Vertex], indices: &[u32]) {
    ...
    while pending.len() > 1 {
        let groups = partition(&clusters, &pending);
        pending.clear();
        ...
        for i in 0..groups.len() {
            ...
            let mut error = 0.0;
            let simplified = simplify2(vertices, &merged, CLUSTER_SIZE * 2 * 3, Some(&mut error));
            if simplified.len() as f64 > merged.len() as f64 * 0.85 || simplified.len() > CLUSTER_SIZE * 3 * 3 {
                #[cfg(feature = "trace")]
                println!(
