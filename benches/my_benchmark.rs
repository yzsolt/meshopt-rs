...
use meshopt_rs::index::generator::{generate_shadow_index_buffer, generate_vertex_remap, remap_index_buffer, remap_vertex_buffer};
...
fn with_input(c: &mut Criterion) {
    let input_name = "alfa147.obj";
    let mesh = Mesh::load(Path::new(input_name)).unwrap();

    c.bench_with_input(BenchmarkId::new("optimize_vertex_cache", input_name), &mesh, |b, mesh| {
        let mut result = mesh.indices.clone();

        b.iter(|| optimize_vertex_cache(&mut result, &mesh.indices, mesh.vertices.len()));
    });

    c.bench_with_input(BenchmarkId::new("optimize_vertex_cache_fifo", input_name), &mesh, |b, mesh| {
    ...
    c.bench_with_input(BenchmarkId::new("optimize_vertex_cache_strip", input_name), &mesh, |b, mesh| {
        let mut result = mesh.indices.clone();
    ...
    c.bench_with_input(BenchmarkId::new("optimize_vertex_fetch", input_name), &mesh, |b, mesh| {
    ...
    c.bench_with_input(BenchmarkId::new("optimize_vertex_fetch_remap", input_name), &mesh, |b, mesh| {
    ...
    c.bench_with_input(BenchmarkId::new("build_meshlets", input_name), &copy, |b, mesh| {
        ...
        b.iter(|| build_meshlets(&mut meshlets, &mesh.indices, mesh.vertices.len(), MAX_VERTICES, MAX_TRIANGLES));
    });

    c.bench_with_input(BenchmarkId::new("compute_meshlet_bounds", input_name), &copy, |b, mesh| {
        const MAX_VERTICES: usize = 64;
        const MAX_TRIANGLES: usize = 126;

        let mut meshlets = vec![Meshlet::default(); build_meshlets_bound(mesh.indices.len(), MAX_VERTICES, MAX_TRIANGLES)];
        build_meshlets(&mut meshlets, &mesh.indices, mesh.vertices.len(), MAX_VERTICES, MAX_TRIANGLES);

        b.iter(|| for meshlet in &meshlets { compute_meshlet_bounds(&meshlet, &mesh.vertices); });
    });

    c.bench_with_input(BenchmarkId::new("generate_shadow_index_buffer", input_name), &copy, |b, mesh| {
    ...
    {
        group.throughput(Throughput::Bytes((mesh.indices.len() * std::mem::size_of::<u32>()) as u64));
        ...
        group.throughput(Throughput::Bytes((mesh.indices.len() * std::mem::size_of::<u32>()) as u64));
        group.bench_with_input(BenchmarkId::new("decode_index_buffer", input_name), &encoded_indices, |b, encoded_indices| {
    ...
    let mut group = c.benchmark_group("index-sequence-encoding");
    {
        ...
        group.bench_with_input(BenchmarkId::new("encode_index_sequence", input_name), &strip, |b, data| {
        ...
        group.throughput(Throughput::Bytes((strip.len() * std::mem::size_of::<u32>()) as u64));
        group.bench_with_input(BenchmarkId::new("decode_index_sequence", input_name), &buffer, |b, buffer| {
